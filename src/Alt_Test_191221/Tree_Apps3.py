import itertools
from Plot_Multi_2columnC import import_ROOT_file
...
class Tree_Comparison_App:
    ...
    def __init__(self,input_dictionary):
        ...
        for i,(k,v) in enumerate(input_dictionary.items()):
            io = create_input_object(index=i,name=k,ROOT_file_path=v["file_path"],tree_name=v["tree_name"])
    ...
    def project_trees(self):
        for io in self.list_of_input_objects:
            for br in self.all_branches:
                io.observables[br] = io.tree[br].array(library="pd")
