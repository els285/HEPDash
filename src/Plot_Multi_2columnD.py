...
class PhysOb_Page_TwoColumn:
    ...
    def Build(self):
        ...
        with col3:
                    ...
                    # Loop over the dataframes and generate the histograms
            for name,df in dic_of_df.items():
                h = generate_wrapped_histogram(page_data["num_bins"],page_data["hist_range"],df,normalise=page_data["normalise"])    
                dic_of_hists[name] = h
            ...
            # print(list(dic_of_df.values())[0])
            # input()
            fig = px.histogram(list(dic_of_df.values())[0], nbins=50,width=800,height=800)
            st.plotly_chart(fig)
